...
# POST Method Tests
    ...
    def test_project_detail_POST_expense_with_no_data(self):
        # if we send an empty response no expense should be added
        response = self.client.post(self.detail_url, {})
        self.assertEquals(self.project1.expenses.count(), 0)

    def test_project_detail_POST_delete_expense(self):
    ...
    def test_project_detail_POST_delete_expense_no_id(self):
        category1 = Category.objects.create(project=self.project1,
                                            name='development')
        Expense.objects.create(project=self.project1,
                               title='expense1',
